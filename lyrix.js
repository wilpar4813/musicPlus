...
function getLyrix(artist, song) {
  ...
  if (artist != "" || song != "") {
    $.ajax({
      ...
      url:
        "https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1/matcher.lyrics.get?format=json&callback=callback&q_track=" +
        song +
        "&q_artist=" +
        artist +
        "&apikey=057fcae6cc1599a783e98bf3f2153ced",
      method: "GET"
    ...
    // Error message if user doesn't enter anything
...
function postToHtml(response) {
  ...
  var myEscapedLyrixString = myLyrixString
    .replace(/\\n/g, "<br>")
  ...
  $("#songLyrix").html(
    myEscapedLyrixString.slice(1, myEscapedLyrixString.length - 1)
  )
