$(document).ready(function() {
  ...
  function callBackSearch() {
    ...
    $.ajax({
      ...
      for (var i = 0; i < response.data.length && i < 10; i++) {
        var titleDiv = $(
          `<div class="column" id="songSpot" data-number=${i} data-preview=${response.data[i].preview} >`
        )
        titleDiv.html(
          "Title: <span id='songTitle'>" + response.data[i].title + "<span>"
        )
  ...
  $("#searchButton").on("click", callBackSearch)
  ...
  function addToPlaylist() {
    ...
    addSongArr.forEach(function(val) {
      ...
      div.attr(
        "data-play",
        "https://img.icons8.com/flat_round/24/000000/play--v1.png"
      )
      div.attr(
        "data-pause",
        "https://img.icons8.com/flat_round/24/000000/pause--v1.png"
      )
      div.html(
        val.title +
          `<span id='songBtn' style='float: right;'>
  ...
  $("#remove").on("click", function() {
    ...
    $("#songRow")
      .children()
      .attr("style", "background: darkgrey;")
